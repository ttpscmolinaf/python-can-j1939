...
class J1939_21:
                ...
                if buf["deadline"] > now:
                    ...
                    if buf["state"] == self.SendBufferState.WAITING_CTS:
                        ...
                        while buf["next_packet_to_send"] < buf["num_packages"]:
                            ...
                            self.__send_tp_dt(
                                buf["src_address"], buf["dest_address"], data
                            )
                            if should_break:
                                break
    ...
    def _process_tp_cm(self, mid, dest_address, data, timestamp, channel):
        ...
        if control_byte == self.ConnectionMode.RTS:
            ...
            if buffer_hash in self._rcv_buffer:
                ...
                self.__send_tp_abort(
                    dest_address, src_address, self.ConnectionAbortReason.BUSY, pgn
                )
                return
            ...
            self.__send_tp_cts(
                dest_address,
                src_address,
                self._rcv_buffer[buffer_hash]["num_packages_max_rec"],
                1,
                pgn,
            )
            self.__job_thread_wakeup()
            ...
            if buffer_hash not in self._snd_buffer:
                self.__send_tp_abort(
                    dest_address, src_address, self.ConnectionAbortReason.RESOURCES, pgn
                )
                return
            if num_packages == 0:
                # SAE J1939/21
            ...
            if next_package_number + num_packages > num_packages_all:
                logger.debug(
                    ...
                    num_packages,
                    num_packages_all - next_package_number,
                )
                num_packages = num_packages_all - next_package_number

            self._snd_buffer[buffer_hash]["next_wait_on_cts"] = (
                self._snd_buffer[buffer_hash]["next_packet_to_send"] + num_packages - 1
            )
    ...
    def _process_tp_dt(self, mid, dest_address, data, timestamp, channel):
        ...
        if (dest_address != ParameterGroupNumber.Address.GLOBAL) and (
            sequence_number >= self._rcv_buffer[buffer_hash]["next_packet"]
        ):
            # send cts
            number_of_packets_that_can_be_sent = min(
                self._rcv_buffer[buffer_hash]["num_packages_max_rec"],
                self._rcv_buffer[buffer_hash]["num_packages"]
                - self._rcv_buffer[buffer_hash]["next_packet"],
            )
            ...
            self._rcv_buffer[buffer_hash]["next_packet"] = min(
                self._rcv_buffer[buffer_hash]["next_packet"]
                + self._rcv_buffer[buffer_hash]["num_packages_max_rec"],
                self._rcv_buffer[buffer_hash]["num_packages"],
            )
    ...
    def __send_tp_cts(
        self, src_address, dest_address, num_packets, next_packet, pgn_value
    ):
        pgn = ParameterGroupNumber(0, 236, dest_address)
        mid = MessageId(
            priority=7, parameter_group_number=pgn.value, source_address=src_address
    ...
    def __send_tp_eom_ack(
        self, src_address, dest_address, message_size, num_packets, pgn_value
    ):
        pgn = ParameterGroupNumber(0, 236, dest_address)
        mid = MessageId(
            priority=7, parameter_group_number=pgn.value, source_address=src_address
    ...
    def __send_acknowledgement(
        self, control_byte, group_function_value, address_acknowledged, pgn
    ):
        data = [
            control_byte,
    ...
    def __send_tp_bam(
        self, src_address, priority, pgn_value, message_size, num_packets
    ):
