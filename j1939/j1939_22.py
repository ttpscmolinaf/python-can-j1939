...
class J1939_22:
    class TpControlType:
        ...
        EOM_STATUS = (
            2  # Destination Specific or Global Destination End_of_Message Status
        )
    ...
    def __init__(
        ...
        self._LUT_FD_DLC = []
        for i in range(9):
            self._LUT_FD_DLC.append(i)
        for _ in range(4):
            self._LUT_FD_DLC.append(12)
        for _ in range(4):
            self._LUT_FD_DLC.append(16)
        for _ in range(4):
            self._LUT_FD_DLC.append(20)
        for _ in range(4):
            self._LUT_FD_DLC.append(24)
        for _ in range(8):
            self._LUT_FD_DLC.append(32)
        for _ in range(16):
            self._LUT_FD_DLC.append(48)
        for _ in range(16):
            self._LUT_FD_DLC.append(64)
    ...
    def send_pgn(
        ...
        if data_length <= self.DataLength.TP:
            ...
            if time_limit == 0:
                ...
                while True:
                    hash = self._buffer_hash_mpg(
                        frame_format, session, src_address, dst_address
                    )
                    # hash = self._buffer_hash(session, src_address, dst_address)
                    if hash not in self._multi_pg_snd_buffer:
                        self._multi_pg_snd_buffer[hash] = {
                            "deadline": deadline,
                            "cpg": [cpg],
                            "fill_level": 4 + data_length,
                        }
                        break
                    elif self._multi_pg_snd_buffer[hash]["fill_level"] <= (
                        self.DataLength.TP - data_length
                    ):
                        # update fill level
            ...
            ) or ParameterGroupNumber(0, pdu_format, pdu_specific).is_pdu2_format:
            ...
            num_segments = int(message_size / self.DataLength.TP) + (
                (message_size % self.DataLength.TP) != 0
            )
            ...
            if dest_address == ParameterGroupNumber.Address.GLOBAL:
                ...
                self.__send_tp_rts(
                    ...
                    message_size,
                    num_segments,
                    min(self._max_cmdt_packets, num_segments),
                )
    ...
    def __send_multi_pg(self, frame_format, cpg_list, src_address, dst_address):
        ...
        for cpg in cpg_list:
            ...
            data.append(
                (cpg["tos"] << 5) | (cpg["tf"] << 2) | ((cpg["cpgn"] >> 16) & 0x3)
            )
        ...
        if frame_format == FrameFormat.FBFF:
            ...
            mid = MessageId(
                priority=priority,
                parameter_group_number=ParameterGroupNumber.PGN.FEFF_MULTI_PG
                | (dst_address & 0xFF),
        ...
        for bufid in list(self._rcv_buffer):
            ...
            if buf["deadline"] != 0:
                if buf["deadline"] > now:
                    ...
                    # deadline reached
                    logger.info(
                        "Deadline reached for rcv_buffer src 0x%02X dst 0x%02X",
                        buf["src_address"],
                        buf["dest_address"],
                    )
                    if buf["dest_address"] != ParameterGroupNumber.Address.GLOBAL:
                        self.__send_tp_abort(
                            ...
                            buf["src_address"],
                            buf["session"],
                            self.ConnectionAbortReason.TIMEOUT,
                            buf["pgn"],
                        )
            ...
            if buf["deadline"] != 0:
                if buf["deadline"] > now:
                    ...
                    if buf["state"] == self.SendBufferState.WAITING_CTS:
                        logger.info(
                            "Deadline WAITING_CTS reached for snd_buffer src 0x%02X dst 0x%02X",
                            buf["src_address"],
                            buf["dest_address"],
                        )
                        self.__send_tp_abort(
                        ...
                        while buf["next_packet_to_send"] < buf["num_segments"]:
                            ...
                            if (package + 1) == buf["num_segments"]:
                                ...
                                buf["deadline"] = (
                                    time.time() + self._minimum_tp_rts_cts_dt_interval
                                )
                                break
                        ...
                        if buf["next_packet_to_send"] < buf["num_segments"]:
                            buf["deadline"] = (
                                time.time() + self._minimum_tp_bam_dt_interval
                            )
                            # recalc next wakeup
                            if next_wakeup > buf["deadline"]:
                                next_wakeup = buf["deadline"]
    ...
    def _process_tp_cm(self, mid, dest_address, data, timestamp):
        ...
        segment_num = (
            (data[4] & 0xFF) | ((data[5] & 0xFF) << 8) | ((data[6] & 0xFF) << 16)
        )
        ...
        if control_byte == self.TpControlType.RTS:
            ...
            num_segments = data[
                7
            ]  # Maximum number of segments that can be sent in response to one CTS.
            ...
            if buffer_hash in self._rcv_buffer:
                ...
                self.__send_tp_abort(
                    ...
                    src_address,
                    session_num,
                    self.ConnectionAbortReason.BUSY,
                    pgn,
                )
            ...
            self.__send_tp_cts(
                ...
                src_address,
                session_num,
                self._rcv_buffer[buffer_hash]["num_segments_max_rec"],
                1,
                pgn,
            )
            self.__job_thread_wakeup()
            ...
            if num_segments == 0:
                ...
                self._snd_buffer[buffer_hash]["deadline"] = (
                    time.time() + self.Timeout.Th
                )
                self.__job_thread_wakeup()
                return
            ...
            segments_to_be_sent = (
                num_segments_all - self._snd_buffer[buffer_hash]["next_packet_to_send"]
            )
            if num_segments > num_segments_all:
                logger.debug(
                    "CTS: Allowed more packets %d than complete transmission %d",
                    num_segments,
                    num_segments_all,
                )
            ...
            if num_segments > self._max_cmdt_packets:
                logger.debug(
                    "CTS: Allowed more packets %d than transmitters max-cmdt-number %d",
                    num_segments,
                    self._max_cmdt_packets,
                )
                num_segments = self._max_cmdt_packets
            if num_segments > segments_to_be_sent:
                logger.debug(
                    "CTS: Allowed more packets %d than needed to complete transmission %d",
                    num_segments,
                    segments_to_be_sent,
                )
                num_segments = segments_to_be_sent

            self._snd_buffer[buffer_hash]["next_wait_on_cts"] = (
                self._snd_buffer[buffer_hash]["next_packet_to_send"] + num_segments - 1
            )

            self._snd_buffer[buffer_hash][
                "state"
            ] = self.SendBufferState.SENDING_RTS_CTS
            self._snd_buffer[buffer_hash][
                "deadline"
            ] = time.time()  # wake up immediately
            self.__job_thread_wakeup()
            ...
            if (self._rcv_buffer[buffer_hash]["message_size"] == message_size) and (
                self._rcv_buffer[buffer_hash]["num_segments"] == segment_num
            ):
                self.__notify_subscribers(
                    ...
                    dest_address,
                    timestamp,
                    self._rcv_buffer[buffer_hash]["data"],
                )
                if dest_address != ParameterGroupNumber.Address.GLOBAL:
                    self.__send_tp_eom_ack(
                        dest_address,
                        src_address,
                        session_num,
                        message_size,
            ...
            self._snd_buffer[buffer_hash][
                "state"
            ] = self.SendBufferState.EOM_ACK_RECEIVED
            self._snd_buffer[buffer_hash][
                "deadline"
            ] = time.time()  # wake up immediately
            self.__job_thread_wakeup()
        ...
        # BAM FD.TP.CM received
            ...
            buffer_hash = self._buffer_hash(session_num, dest_address, src_address)
            if (
                buffer_hash in self._snd_buffer
                and self._snd_buffer[buffer_hash]["state"]
                == self.SendBufferState.WAITING_CTS
            ):
                # cancel transmission
                self._snd_buffer[buffer_hash][
                    "state"
                ] = self.SendBufferState.TRANSMISSION_FINISHED
                self._snd_buffer[buffer_hash]["deadline"] = time.time()
            # TODO: any more abort responses?
        else:
            raise RuntimeError(
                "Received TP.CM with unknown control_byte %d", control_byte
            )
    ...
    def _process_tp_dt(self, mid, dest_address, data, timestamp):
        ...
        if self._rcv_buffer[buffer_hash]["next_packet"] != segment_num:
            logger.critical(
                "packet error. required: "
                + str(self._rcv_buffer[buffer_hash]["next_packet"])
                + " received: "
                + str(segment_num)
            )
            return
        ...
        if (
            len(self._rcv_buffer[buffer_hash]["data"])
            >= self._rcv_buffer[buffer_hash]["message_size"]
        ):
            logger.info(
                "finished RCV of PGN {} with size {}".format(
                    self._rcv_buffer[buffer_hash]["pgn"],
                    self._rcv_buffer[buffer_hash]["message_size"],
                )
            )
            # shorten data to message_size
            self._rcv_buffer[buffer_hash]["data"] = self._rcv_buffer[buffer_hash][
                "data"
            ][: self._rcv_buffer[buffer_hash]["message_size"]]
            # finished reassembly
            if dest_address != ParameterGroupNumber.Address.GLOBAL:
                # set deadlin for waiting on eom status
                self._rcv_buffer[buffer_hash]["deadline"] = (
                    time.time() + self.Timeout.T1
                )
            self.__job_thread_wakeup()
            return

        # send clear to send
        if (dest_address != ParameterGroupNumber.Address.GLOBAL) and (
            segment_num >= self._rcv_buffer[buffer_hash]["next_cts_border"]
        ):
            # send cts
            number_of_packets_that_can_be_sent = min(
                self._rcv_buffer[buffer_hash]["num_segments_max_rec"],
                self._rcv_buffer[buffer_hash]["num_segments"]
                - self._rcv_buffer[buffer_hash]["next_cts_border"],
            )
            next_packet_to_be_sent = (
                self._rcv_buffer[buffer_hash]["next_cts_border"] + 1
            )
            self.__send_tp_cts(
                dest_address,
                src_address,
                session_num,
                number_of_packets_that_can_be_sent,
                next_packet_to_be_sent,
                self._rcv_buffer[buffer_hash]["pgn"],
            )

            # calculate next packet number at which a CTS is to be sent
            self._rcv_buffer[buffer_hash]["next_cts_border"] = min(
                self._rcv_buffer[buffer_hash]["next_cts_border"]
                + self._rcv_buffer[buffer_hash]["num_segments_max_rec"],
                self._rcv_buffer[buffer_hash]["num_segments"],
            )
    ...
    def __send_tp_abort(
        self, src_address, dest_address, session_num, reason, pgn_value
    ):
        self.__send_tp_cm(
            src_address,
            dest_address,
            self.TpControlType.ABORT,
            session_num,
            0xFFFFFF,
            0xFFFFFF,
            0xFFFFFF,
    ...
    def __send_tp_eom_status(
        ...
        num_segments,
        pgn_value,
        size_of_assurance_data=0,
        adt=Adt.NO_ADT,
    ):
        self.__send_tp_cm(
            src_address,
            dest_address,
            self.TpControlType.EOM_STATUS,
            session_num,
            message_size,
            num_segments,
            size_of_assurance_data,
            adt,
            pgn_value,
        )

    def __send_tp_eom_ack(
        self,
        src_address,
        dest_address,
        session_num,
    ...
    def __send_tp_bam(
        self, priority, src_address, session_num, pgn_value, message_size, num_segments
    ):
        self.__send_tp_cm(
            src_address,
            ParameterGroupNumber.Address.GLOBAL,
            self.TpControlType.BAM,
            session_num,
            message_size,
            num_segments,
            0xFF,
    ...
    def __send_tp_cm(
        ...
        pgn_tp_cm = ParameterGroupNumber(
            0, (ParameterGroupNumber.PGN.FD_TP_CM >> 8) & 0xFF, dest_address
        )
        mid = MessageId(
            priority=priority,
            parameter_group_number=pgn_tp_cm.value,
            source_address=src_address,
        )
    ...
    def __send_tp_dt(
        self, src_address, dest_address, session_num, segment_num, data, Dtfi=0
    ):
        pgn = ParameterGroupNumber(
            0, (ParameterGroupNumber.PGN.FD_TP_DT >> 8) & 0xFF, dest_address
        )
        mid = MessageId(
            priority=7, parameter_group_number=pgn.value, source_address=src_address
        )
    ...
    def notify(self, can_id, data, timestamp, channel):
